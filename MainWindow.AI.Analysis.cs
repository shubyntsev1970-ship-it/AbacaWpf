...
{
    // Агрессивный режим: отдельная оценка "гонки за приз".
    // Она включается только с первой раздачи и только для легких строк, которые реально можно закрывать сериями:
    // сумма, пара, две пары и тройка. Абак/каре/фул/стриты уже обрабатываются как премиальные готовые ходы.
    private int GetAggressiveOpeningPrizeRaceMove(Dictionary<int, int> counts)
    {
        if (CurrentPlayer.Difficulty != ComputerDifficulty.Aggressive || _rollCount != 1)
            return -1;
        ...
        return new[] { 14, 6, 7, 8 }
            ...
            .Select(row => new
            {
                ...
                Priority = GetAggressiveOpeningPrizeRacePriority(row, counts)
            })
            .Where(item => item.Score > 0 && item.Priority > int.MinValue / 2)
    ...
    private int GetAggressiveOpeningPrizeRacePriority(int row, Dictionary<int, int> counts)
    {
        if (!IsAggressivePrizeRaceRow(row) || !HasAggressiveReadyResult(row, counts))
            return int.MinValue / 2;

        var column = CurrentPlayer.GetFreeCell(row);
        var ownRowRace = CompletesOwnRowPrize(row);
        var opponentRowRace = OpponentCanClaimRowPrizeSoon(row);
        var ownColumnRace = column >= 0 && column < ColumnCount - 1 && CompletesOwnColumnPrize(column);
        var opponentColumnRace = column >= 0 && column < ColumnCount - 1 && OpponentCanClaimColumnPrizeSoon(column);

        // Если приз прямо сейчас не закрывается и не блокируется, агрессивная запись нужна только
        // когда строка уже хорошо набрана и соперник еще может потерять зеркальный приз.
        var rowFill = CountBusyMainCellsInRow(row);
        var isRowPressure = rowFill >= 3 && OpponentPrizeIsOpen(row, ColumnCount - 1);
        if (!ownRowRace && !opponentRowRace && !ownColumnRace && !opponentColumnRace && !isRowPressure)
            return int.MinValue / 2;

        var priority = CalculateScore(row);
        if (ownRowRace)
            priority += 170;
        if (opponentRowRace)
            priority += 150;
        if (ownColumnRace)
            priority += 105;
        if (opponentColumnRace)
            priority += 90;
        if (isRowPressure)
            priority += 55 + rowFill * 12;

        priority += row switch
        {
            14 => 18, // сумма легко закрывает строку серией раздач
            6 => 14,
            7 => 12,
            8 => 10,
            _ => 0
    ...
    private static bool IsAggressivePrizeRaceRow(int row) => row is 6 or 7 or 8 or 14;

    private bool HasAggressiveReadyResult(int row, Dictionary<int, int> counts)
    {
        return row switch
        {
            14 => CalculateScore(14) > 0,
            6 => PairScore(counts) > 0,
            7 => TwoPairsScore(counts) > 0,
